...
namespace Dahomey.Cbor.Serialization
{
    ...
    public ref struct CborReader
    {
        ...
        /// <summary>
        /// Reads a byte string and returns the content as a byte array.
        /// For indefinite-length byte strings, avoids the extra copy that
        /// ReadByteString().ToArray() would incur.
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public byte[] ReadByteStringToArray()
        {
            SkipSemanticTag();
            Expect(CborMajorType.ByteString);
            int size = ReadSize();
            if (size == -1)
            {
                return ReadIndefiniteByteStringToArray();
            }
            return ReadBytes(size, allowScratchBuffer: false).ToArray();
        }
        ...
        private byte[] ReadIndefiniteByteStringToArray()
        {
            byte[]? rented = null;
            int totalLength = 0;
